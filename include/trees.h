/*
 * trees.h
 * Created by T.Gonzalo on 05/06/2014.
...
namespace Tomb
{
	...
	/* Constructor 2, from string */
	template <class TYPE> Tree<TYPE>::Tree(std::string id) {
		...
		AddBranch(Tree<TYPE>(str1));
			
		_nbranches = 0;
		
		_depth = calculate_depth();*/
	}
	...
	*/
	...
	template <class TYPE> bool Tree<TYPE>::operator==(const Tree<TYPE> &aTree) const {
		if(this->Object() == aTree.Object() and this->Branches() == aTree.Branches()) {
			return true;
		}
		return false;
	}
	...
	/* Overloaded != operator */
	template <class TYPE> bool Tree<TYPE>::operator!=(const Tree<TYPE> &aTree) const {
		if(this->Object() != aTree.Object() or this->Branches() != aTree.Branches()) {
			return true;
		}
		return false;
	}
